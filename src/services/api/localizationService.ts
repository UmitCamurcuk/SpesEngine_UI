...
const localizationService = {
  ...
  getTranslations: async (lang: string) => {
    try {
      const response = await axios.get(`${API_URL}/localizations/${lang}`, {
        headers: {
      ...
      console.error('Çeviriler getirilirken hata oluştu:', error);
      throw error;
    }
  },
  ...
  upsertTranslation: async (data: { key: string, namespace?: string, translations: Record<string, string> }) => {
    try {
      ...
      console.error('Çeviri eklenirken/güncellenirken hata oluştu:', error);
      throw error;
    }
  }
};
