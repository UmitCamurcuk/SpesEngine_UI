...
import type { Permission, UpdatePermissionDto } from '../../../services/api/permissionService';
...
const PermissionDetailsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPermissionDetails = async () => {
      ...
      try {
        const permissionData = await permissionService.getPermissionById(id);
        ...
        setFormData({
          ...
          permissionGroup: typeof permissionData.permissionGroup === 'string' 
            ? permissionData.permissionGroup 
            : permissionData.permissionGroup._id,
          isActive: permissionData.isActive
        });
      } catch (err: any) {
        setError('İzin verileri yüklenirken bir hata oluştu: ' + (err.message || err));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedPermission = await permissionService.getPermissionById(id);
      ...
      setError(err.message || 'İzin güncellenirken bir hata oluştu');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm('Bu izni silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.')) {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        setError(err.message || 'İzin silinirken bir hata oluştu');
        setIsLoading(false);
      }
    }
  };
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              {permission?.name} izninin detaylı bilgileri
            </p>
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
        ...
        {success && (
          <div className="mb-4 bg-green-50 dark:bg-green-900/20 p-4 rounded-md border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 flex items-start">
            ...
            <span>İzin başarıyla güncellendi!</span>
          </div>
        )}
        ...
        {isEditing ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* İsim */}
              <div>
                <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400">İzin Adı</h4>
                <p className="mt-1 text-gray-900 dark:text-white">{permission?.name}</p>
              </div>
              
              {/* Kod */}
              <div>
                <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400">İzin Kodu</h4>
                <p className="mt-1 text-gray-900 dark:text-white font-mono text-sm bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded inline-block">{permission?.code}</p>
              </div>
              
              {/* İzin Grubu */}
              <div>
                <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400">İzin Grubu</h4>
                <p className="mt-1 text-gray-900 dark:text-white">
                  {typeof permission?.permissionGroup === 'object' 
                    ? `${permission?.permissionGroup.name} (${permission?.permissionGroup.code})` 
                    : 'Belirtilmemiş'
                  }
                </p>
              </div>
              ...
              <div className="col-span-1 md:col-span-2">
                ...
                <p className="mt-1 text-gray-900 dark:text-white whitespace-pre-wrap">{permission?.description || '-'}</p>
              </div>
